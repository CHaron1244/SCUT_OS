...
class INode{
    ...
    int type;//表示文件类型，0为目录，1为文件
    int AdAccess[3];//管理员文件保护类型，分别表示可读/可写/可执行
    int UserAccess[3];//普通用户文件保护类型，分别表示可读/可写/可执行
    char ParNode[32];//存放父结点，32表示的是父节点的地址位数，全0表示没有父节点 
    char ChildNode[3][32];//存放子结点，文件没有子结点，每个目录下最多有3个子节点 
    ...
    INode(char* FN, int t, int* AdAcc, int* UserAcc, char* PNode, char** CNode, char* DAdd)  //有参初始化
    {
    	// 设置文件名字 
        for (int i = 0; i < 8; i++)
            FileName[i] = FN[i];
...
// 主函数 
int main(){
	fstream Disk_Pointer("..\\disk.txt", ios::in | ios::out); //文件流对象
    Init(Disk_Pointer);
    ...
    while (1)
    {
        ...
        if (strlen((char*)InstructionRec)){//判断共享内存中是否有内容
            ...
            if (rdIsNotEmpty == 1){
                ...
                string INodeSqe;
                string DocSqe;
                DocSysHost = FindDocName(Disk_Pointer, 0); // 文件系统归属
                JumpPointer(Disk_Pointer, 0, 64);
                DocSysSize = to_string(Add2BlockSqe(Output32Bit(Disk_Pointer))); // 文件系统大小
                JumpPointer(Disk_Pointer, 0, 96);
                DiskBlockSize = to_string(Add2BlockSqe(Output32Bit(Disk_Pointer))); // 磁盘块大小
                JumpPointer(Disk_Pointer, 0, 128);
                SuperBlockSqe = to_string(Add2BlockSqe(Output32Bit(Disk_Pointer))); // 超级块序号
                JumpPointer(Disk_Pointer, 0, 160);
                BitMapSqe = to_string(Add2BlockSqe(Output32Bit(Disk_Pointer))); // 位图序号
                JumpPointer(Disk_Pointer, 0, 192);
                RootSqe = to_string(Add2BlockSqe(Output32Bit(Disk_Pointer))); // 根目录序号
                JumpPointer(Disk_Pointer, 0, 224);
                INodeSqe = to_string(Add2BlockSqe(Output32Bit(Disk_Pointer))); // i结点区序号
                JumpPointer(Disk_Pointer, 0, 256);
                DocSqe = to_string(Add2BlockSqe(Output32Bit(Disk_Pointer))); // 文件区序号
                ReplyInfo = ReplyInfo+"文件系统所有者:"+DocSysHost + "\n";
                ReplyInfo = ReplyInfo + "硬盘大小:" + DocSysSize +"MB" + "\n";
                ...
                ReplyInfo = ReplyInfo + "I结点区剩余空间:" + to_string(GetINodeRemain(Disk_Pointer)) + "B\n";
                ReplyInfo = ReplyInfo + "文件区序号:" + DocSqe + "\n";
                ReplyInfo = ReplyInfo + "文件区剩余空间:" + to_string(GetDocRemain(Disk_Pointer)) + "B";
...
void Init(fstream& Disk_Pointer){
    ...
    char RootDirName[] = "root0000";//根目录名
    ...
    InputCharArr(Disk_Pointer, const_cast<char*>("tyd")); // 文件系统归属 
    JumpPointer(Disk_Pointer, 0, 64);
    InputIntArr(Disk_Pointer, BlockSqe2Add(100), 32); // 位图大小，有100块 
    JumpPointer(Disk_Pointer, 0, 96);
    InputIntArr(Disk_Pointer, BlockSqe2Add(1024), 32); // 磁盘块大小 
    JumpPointer(Disk_Pointer, 0, 128); // 更改 
    InputIntArr(Disk_Pointer, BlockSqe2Add(0), 32); // 超级块序号 
    JumpPointer(Disk_Pointer, 0, 160);
    InputIntArr(Disk_Pointer, BlockSqe2Add(1), 32); // 位图序号 
    JumpPointer(Disk_Pointer, 0, 192);
    InputIntArr(Disk_Pointer, BlockSqe2Add(101), 32); // 根目录序号 
    JumpPointer(Disk_Pointer, 0, 224);
    InputIntArr(Disk_Pointer, BlockSqe2Add(102), 32); // i节点区序号 
    JumpPointer(Disk_Pointer, 0, 256);
    InputIntArr(Disk_Pointer, BlockSqe2Add(50102), 32); // 文件区序号 
...
int GetBlockSqe(fstream& Disk_Pointer){
    int PointerSqe = Disk_Pointer.tellg();//tellg获取文件流对象当前读取位置,返回一个表示文件指针位置的整数值
...
void BitMapChange(fstream& Disk_Pointer, int BlockSqe, int Tag){
    ...
    Disk_Pointer << Tag;
    JumpPointer(Disk_Pointer, OriSqe, 0); // 更改 
}

// 输入函数
void InputCharArr(fstream& Disk_Pointer, char* CharArr){//把每个字符数据的8位二进制表示写入当前位置(写入这个字符数组)
    for (int i = 0; i < strlen(CharArr); i++)
        InputChar(Disk_Pointer, CharArr[i]);
    return;
} 

void InputChar(fstream& Disk_Pointer, char Char){//把Char这个字符的8位二进制表示写入当前位置(写入这个字符)
...
void InputIntArr(fstream& Disk_Pointer, int* IntArr, int Length){
    for (int i = 0; i < Length; i++)
        Disk_Pointer << IntArr[i];//向当前位置写入数组中的数据(0 or 1)
    return;
}

void InputDocName(fstream& Disk_Pointer,int BlockSqe,string DocName){//存文件名
    int OriSqe = GetBlockSqe(Disk_Pointer);
...
int* FindDocComAcc(fstream& Disk_Pointer, int BlockSqe){
    ...
    int* ComAcc;
    ComAcc = new int[3];
    JumpPointer(Disk_Pointer, BlockSqe, 77); // 更改
    for (int i = 0; i < 3; i++){
        Disk_Pointer >> TempChar;
...
string CopyNewFile(fstream& Disk_Pointer,istringstream& Buf){
    ...
    string CurDirName = newfileAddLocation(Disk_Pointer, newfileNameBuf); // 更改
    /*文件名格式检测*/
    string ReplyInfo = FileNameTest(CurDirName);
    if (ReplyInfo != "")
        return ReplyInfo;
