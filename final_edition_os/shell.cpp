...
void LogIn()
{
    ...
    cin >> password;
    if (userName.compare("admin") == 0 && password.compare("123") == 0)
        SysUserInteractiveUI(); // interactive interface
    else if (userName.compare("user") == 0 && password.compare("321") == 0)
...
void ComUserInteractiveUI()
{
    ...
    while (1) {
        ...
        HANDLE WriteSpace = OpenFileMapping(FILE_MAP_ALL_ACCESS, NULL, "WriteSpace"); // continuously get data from the reply information space
        ...
        if (InputFile == WritingFile && InputInstruction == "write") {
            cout << "The current file is being written by another user. Please try again later." << endl;
            cout << ReplyInfoRecBuffer << endl;
            char NULLChar[] = ""; // Clear input
            strcpy_s((char*)InstructionSend, strlen(NULLChar) + 1, NULLChar);
            continue;
        }

        while (1) {
            HANDLE ReplyInfoSpace = OpenFileMapping(FILE_MAP_ALL_ACCESS, false, "ReplyInfoSpace"); // continuously getting data from the response information space
