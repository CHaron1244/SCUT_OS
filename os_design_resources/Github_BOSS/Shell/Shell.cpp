#include <iostream>
#include <graphics.h>  //图形编程提供了一些函数和数据结构，用于在屏幕上创建图形和进行基本的绘图操作。在C++中，它通常与BGI（Borland Graphics Interface）图形库一起使用，用于在窗口上绘制图形
#include <fstream>  //文件输入和输出操作
#include <windows.h>  //包含了用于Windows操作系统编程的一些函数和宏定义。它允许您在Windows环境中进行各种系统级别的操作，如窗口管理、消息处理等
#include <sstream>  //用于字符串流操作。提供了stringstream类，允许您将字符串作为流处理，执行字符串和数值之间的转换操作。对于将数字转换为字符串或从字符串中提取数字非常有用。
