...
int main(){
    fstream Disk_Pointer("E:\\Desktop\\大三上\\!!!!!!!!!!!!操作系统课程设计\\cys.txt", ios::in | ios::out); //文件流对象
    Init(Disk_Pointer);
...
void Init(fstream& Disk_Pointer){
    /*初始化根目录*/
    Disk_Pointer.seekg(1024 * 101, ios::beg);//将文件流对象 Disk_Pointer 中的文件指针移动到文件的开头位置再偏移101KB的位置处
...
void InputCharArr(fstream& Disk_Pointer, char* CharArr){//把字符数据的8位二进制表示写入当前位置(写入这个字符数组)
